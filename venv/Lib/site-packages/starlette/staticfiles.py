...
class StaticFiles:
    ...
    async def check_config(self) -> None:
        """
        Perform a one-off configuration check that static is actually
        ...
        try:
            ...
            raise RuntimeError(
                f"static directory '{self.directory}' does not exist."
            )
        if not (stat.S_ISDIR(stat_result.st_mode) or stat.S_ISLNK(stat_result.st_mode)):
            raise RuntimeError(
                f"static path '{self.directory}' is not a directory."
            )
